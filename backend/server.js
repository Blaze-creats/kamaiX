...
// Real email OTP: sign up at msg91.com, create an OTP template with channel = Email
// (no DLT registration needed for email, unlike SMS), then set these as environment
...
const EMAIL_CONFIGURED = Boolean(MSG91_AUTH_KEY && MSG91_EMAIL_TEMPLATE_ID);

function generateOtp() {
  return String(Math.floor(100000 + Math.random() * 900000));
}

// Sends a real OTP email via MSG91. Returns true if MSG91 accepted the request.
// Throws on network error so the caller can fall back gracefully.
// NOTE: double-check the exact endpoint/param names in your MSG91 dashboard docs —
// their email-OTP API shape can differ slightly by account/plan.
async function sendRealEmail(email, otp) {
  const url = `https://api.msg91.com/api/v5/otp?template_id=${MSG91_EMAIL_TEMPLATE_ID}&email=${encodeURIComponent(email)}&authkey=${MSG91_AUTH_KEY}&otp=${otp}`;
